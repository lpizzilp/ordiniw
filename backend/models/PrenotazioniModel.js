...
export const getsum = (result) => {
    db.query("SELECT f.food_id, f.food_name, SUM(bd.item_qty) AS somma_qty FROM food AS f JOIN bookdetails bd ON f.food_id = bd.food_id JOIN bookstatus bs ON bd.book_id = bs.book_id GROUP BY f.food_name ORDER BY f.food_name asc", (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }
